...
class WP_TripAdvisor_Review_Admin {
	...
	public function wptripadvisor_tripadvisor_section_developers_cb($args)
	{
		...
		echo "<p>Use this page to download your newest 10 TripAdvisor business reviews and save them in your Wordpress database. They will show up on the Review List page once downloaded. Any new reviews you get are downloaded once a day. The Pro version can download all of your reviews from multiple locations.</p><p>Note: The free version does not currently work for Vacation Rentals. The Pro version does using the new Review Funnels feature.</p>";
	}
...
//used to find showeruserreview url
		...
		// Create DOM from URL or file
			...
			if($vactionrental==true){
				if($html->find('div.reviewSelector')){
					...
					echo "Error 102a: Unable to read Vacation Rental TripAdvisor page.";
					die();	
				}
			} else {
			...
			if(isset($reviewobject) && $reviewobject5!="" && $nextbtnlink==''){
		...
		$HotelUserReviews = false;
			...
			if (filter_var($options['tripadvisor_business_url'], FILTER_VALIDATE_URL)) {
			  ...
			  if($options['tripadvisor_radio']=='yes'){
				...
				if (strpos($stripvariableurl, 'Hotel_Review') !== false || strpos($stripvariableurl, 'VacationRentalReview') !== false ) {
					//this is a hotel or vacation rental review, we have to now fine the ShowUserReviews url
